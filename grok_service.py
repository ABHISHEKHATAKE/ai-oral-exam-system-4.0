...
async def generate_questions_with_grok(pdf_text: str, instructions: str, num_questions: int = 5) -> List[dict]:
    """
    Generate questions using Grok API based on PDF content and instructions
    """
    
    # Truncate PDF text to avoid token limits (keep first 3000 chars)
    # This is approximately 600-750 tokens
    truncated_text = pdf_text[:3000] if len(pdf_text) > 3000 else pdf_text
    
    print(f"\n{'='*60}")
    print(f"Generating questions with Grok API...")
...
Return ONLY the JSON array, no additional text."""
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            print(f"Received response from Grok (length: {len(content)} chars)")
...
async def evaluate_answer_with_grok(question: dict, answer: str) -> dict:
    """
    Evaluate a student's answer using Grok API
    """
